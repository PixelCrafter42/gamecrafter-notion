...
在「博客文章」数据库管理文章和项目，状态为「已发布」才会出现在网站。“内容类型”选择“文章”时进入写作归档，选择“项目”时进入项目页。
...
站点名称、简介、作者资料、头像、邮箱、社交链接、导航名称、首页/写作/项目/关于页文案及栏目显示开关统一放在 Notion 的“站点配置 · gamecrafter.fun”数据库。唯一配置记录的页面正文就是网站“关于”正文。修改属性会由 Webhook 自动发布；修改“关于”正文后使用管理页“全量更新”。头像和正文附件会下载到站点资源中，不依赖会过期的 Notion 文件地址。`src/site.config.ts` 只负责读取同步生成的配置并固定正式域名。

项目与文章复用同一个发布按钮和增量机制。项目可填写状态、项目类型、项目主页、代码仓库、封面和“精选”；精选项目优先出现在首页。
