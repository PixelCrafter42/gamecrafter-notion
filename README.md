...
「博客后台 · gamecrafter.fun」是统一的多数据源数据库。通过顶部标签进入「写作」「项目」「站点设置」和「栏目管理」。写作和项目拥有各自字段，状态为「已发布」才会出现在网站。
...
项目与文章使用独立数据源，但复用同一个发布按钮和增量机制。项目可填写状态、项目类型、项目主页、代码仓库、封面和“精选”；精选项目优先出现在首页。
