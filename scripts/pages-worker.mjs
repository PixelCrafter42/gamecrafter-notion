...
const articlePaths = new Set(["ARTICLE_PATHS"]);

export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    const missingArticle = url.pathname.startsWith('/blog/') &&
      !['/blog/', '/blog/index.html'].includes(url.pathname) && !articlePaths.has(url.pathname);
    if (missingArticle) url.pathname = '/404.html';
    ...
    const response = new Response(request.method === 'HEAD' ? null : asset.body, {
      status: missingArticle ? 404 : asset.status,
    ...
    if (missingArticle) response.headers.delete('Location');
