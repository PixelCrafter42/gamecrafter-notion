...
export async function collectSnapshot({ request, config, media }) {
  ...
  const links = new Map(published.map(({page,data})=>[page.id,config.site+'/blog/'+data.id+'/']));
  ...
  for (const {page,data} of published) {
    const html = await renderBlocks(await children(page.id), {children,media,publishedLinks:links});
...
export function createRequest({token,cliScript,fetchImpl=fetch}) {
  ...
  return async (path, {method='GET',body} = {}) => {
    ...
    if (!token) throw new Error('缺少 NOTION_TOKEN，请在 GitHub Actions repository secrets 中配置');
    for (let attempt=0; attempt<5; attempt++) {
      ...
      throw new Error('Notion API 请求失败 ('+response.status+')；请检查连接权限和数据库配置');
    }
  };
}
...
export async function runSync() {
  ...
  const media = async body => {
    ...
    assets.set(name,bytes); return '/notion-media/'+name;
  };
  // Finish every API call before changing any generated file.
  const posts = await collectSnapshot({request,config,media});
  ...
  await writeFile(resolve(root,'public/_notion-sync.json'),JSON.stringify({syncedAt})+'\n');
  ...
  console.log('Notion 同步完成：'+posts.length+' 篇已发布文章，'+assets.size+' 个附件。');
}
